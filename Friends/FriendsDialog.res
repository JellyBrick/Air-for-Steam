Friends/FriendsDialog.res {
	...
	layout {
		region { name=header height=110 width=max }
			place [!$OSX] { control=frame_minimize,frame_close height=22 width=22 y=13 align=right margin-right=20 region=header }
			place [$OSX] { control=frame_close,frame_minimize height=22 width=22 y=13 align=left margin-left=20 region=header }
			place { control=frame_captiongrip width=max height=max region=header }
